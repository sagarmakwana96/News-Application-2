...
function articleSection(article,source){
    ...
    if(source=="gd"){
        ...
        try{
            section=article.section.toUpperCase();
        }
        catch(err){}
    }
    if(section=="SPORT")
...
class Home extends React.Component{
    constructor(props){
        ...
        if(localStorage.getItem('source')==null){
            ...
            localStorage.setItem('source', 'gd');
    ...
    componentDidMount = () => {
        ...
        {
            url = "http://newsapp-sagarmakwana96.us-east-1.elasticbeanstalk.com/home/"+false;
            fetch(url)
        ...
        else if(this.state.source == 'gd'){
            var url = "http://newsapp-sagarmakwana96.us-east-1.elasticbeanstalk.com/home/"+true;
            // var url = "https://content.guardianapis.com/search?api-key=d53c9e4d-5261-4e55-ada8-2dcd03e8af91&section=(sport|business|technology|politics)&show-blocks=all";
            fetch(url)
            .then( response => response.json())
            .then(
                // handle the result
    ...
    componentDidUpdate = (prevProps,prevState) => {
        ...
        if(this.state.source !== prevState.source)
        {
            if(this.state.source == 'ny')
            {
                ...
                url = "http://newsapp-sagarmakwana96.us-east-1.elasticbeanstalk.com/home/"+true;
                                    ...
                                    <Link className='cardAnchor' to= {articlePageUrl(article,this.state.source)}>
                                        <div style={{padding:"15px"}}>
                                            ...
                                            <div className='article_description'>
                                                ...
                                                <p><i>{articleDate(article,this.state.source)}</i><span className={sectionColor(articleSection(article,this.state.source))}>{articleSection(article,this.state.source)}</span></p>
                                            </div>
                                        </div>
                                    </Link> 
                                </div>            
